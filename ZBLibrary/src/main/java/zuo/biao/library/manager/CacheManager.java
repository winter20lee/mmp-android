...
public class CacheManager {
	...
	public <T> List<String> getIdList(Class<T> clazz, String group) {
		...
		return sp == null ? null : GsonUtil.jsonToList(sp.getString(StringUtil.getTrimedString(group), null), String.class);
	}
	...
	public <T> void saveList(Class<T> clazz, String group, LinkedHashMap<String, T> map, int start, int pageSize) {
		...
		if (StringUtil.isNotEmpty(group, true)) {
			...
			//id列表
			List<String> idList = GsonUtil.jsonToList(sp.getString(group, null), String.class);
			if (idList == null) {
			...
			editor.remove(group).putString(group, GsonUtil.GsonString(idList)).commit();
	...
	public <T> void save(Class<T> clazz, T data, String id, String group) {
		...
		if (sp != null) {
			...
			if (idList.contains(id) == false) {
				...
				idList.add(0, id);
				sp.edit().remove(group).putString(group, GsonUtil.GsonString(idList)).commit();
			}
		}
	}
